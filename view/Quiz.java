...
public class Quiz extends JFrame implements ActionListener {
    ...
    String name,courseName,courseId,email;

    Quiz(String userName, String course,String email) {
    ...
    public void fetchQuestionsAndAnswersFromDatabase() {
        // Code to fetch questions and answers from the database using QuestionsDao class
        QuestionsDao dbHandler = new QuestionsDao();
        questionsAndAnswersList = dbHandler.fetchQuestionsAndAnswers(courseName);
    }
    
public void shuffleQuestionsAndOptions() {
    ...
    public void quizResult(){
        // Define the format for the date and time
            ...
            Result theResult = new Result();
                theResult.setCourseId(courseId); 
                theResult.setUserName(name);
                theResult.setEmail(email);
                theResult.setCourseName(courseName);
                theResult.setScore(score);
                theResult.setDate(dateTimeString); 
            
            //Creating Result Dao object
            ResultDao dao = new ResultDao();
            
            Integer rowsAffected = dao.insertResult(theResult);
            
            if(rowsAffected != null){
                JOptionPane.showMessageDialog(this, "This Marks the end of the quiz,Click Ok to see your Score");
                setVisible(false);
                new ScoreDashboard(name,score,email).setVisible(true);
            }else{
